...
static double get_slope(t_cub *data, t_vector *dir)
{
	...
	x = data->player.x - (data->player.dir_y * ((double) VIEW_PLANE_SIZE * 2 / WINDOW_WIDTH) * data->render.i) + data->player.dir_x * VIEW_PLANE_DISTANCE;
	y = data->player.y + (data->player.dir_x * ((double) VIEW_PLANE_SIZE * 2 / WINDOW_WIDTH) * data->render.i) + data->player.dir_y * VIEW_PLANE_DISTANCE;
...
static void draw(t_cub *data, t_vector x_ray, t_vector y_ray, t_vector dir)
{
	if (pythagorean(x_ray.x - data->player.x, x_ray.y - data->player.y) <
		...
		draw_column(data, find_intersection(data, x_ray.x, x_ray.y), data->render.i + WINDOW_WIDTH / 2,
					dir.x < 0 ? WEST : EAST, x_ray.y - (int) x_ray.y);
	else
		draw_column(data, find_intersection(data, y_ray.x, y_ray.y), data->render.i + WINDOW_WIDTH / 2,
					dir.y < 0 ? NORTH : SOUTH, y_ray.x - (int) y_ray.x);
}

static double innit_rays(t_cub *data, t_vector *x_ray, t_vector *y_ray, t_vector *dir)
{
	double slope;
...
void draw_rays(t_cub *data)
{
	...
	while (data->render.i < (int) ceil((double) WINDOW_WIDTH / 2))
	{
		...
		while (((int) x_ray.x >= 0 && (int) x_ray.x < MAX_MAP_WIDTH && (int) x_ray.y >= 0 &&
		...
		while ((int) y_ray.x >= 0 && (int) y_ray.x < MAX_MAP_WIDTH && (int) y_ray.y >= 0 &&
			   (int) y_ray.y < MAX_MAP_HEIGHT && (data->map[(int) y_ray.y - (dir.y < 0)][(int) y_ray.x] != '1'))
		{
			y_ray.x += dir.y / slope;
			y_ray.y += dir.y;
		}
