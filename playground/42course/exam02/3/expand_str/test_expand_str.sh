...
run_test() {
    ...
    result=$(./expand_str "$@" | cat -e)

    # Compare the expected and actual results using printf for newline handling
    if [ "$(printf '%s' "$result")" == "$(printf '%s' "$expected")" ]; then
