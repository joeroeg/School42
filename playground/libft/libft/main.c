...
// ft_strlcpy	    int main(void) {     char str_src[6] = "hello";     char str_dst[6];     size_t len;       len = ft_strlcpy(str_dst, str_src, 6);       printf("String: %s\nLength: %zu\n", str_dst, len);     return 0; }
// ft_strlcat	    int main(void) {     char src[50] = "";     char dest[50] = "Hello";     size_t dstsize = 0;     printf("%lu\n", ft_strlcat(dest, src, dstsize));     return 0;}
...
// ft_strchr	    int main(void) {  char string[] = "This is a sample string";  char ch = 'S';  char *ptr = ft_strchr(string, ch);  printf("original string: %s \n", string);  printf("after first letter |%c| in the original string output is the rest of a string |%s|\n", ch, ptr);  return 0;}
...
int	main(void){	char str[] = "Hello World!";	int fd = 1;	ft_putstr_fd(str, fd);	return (0);}
