...
# For compilation with Makefile: ./valgrind_check.sh make /path/to/project target_name [additional_args]
# For using an existing executable: ./valgrind_check.sh exec /path/to/project executable_name [additional_args]

# Check if sufficient arguments are provided
if [ $# -lt 3 ]; then
    echo "Usage: $0 <mode: make|exec> <path_to_project> <makefile_or_executable_name> [additional_args]"
    exit 1
fi
...
# Shift the first three arguments to get any additional ones
shift 3
ADDITIONAL_ARGS="$@"
...
# Run Valgrind with additional arguments and handle output
LOG_FILE="$LOG_DIR/valgrind_log_$(date +%Y%m%d_%H%M%S).txt"
echo "Running Valgrind on $EXECUTABLE_PATH with arguments: $ADDITIONAL_ARGS..."

if [ $LOG_TO_FILE -eq 1 ]; then
    VALGRIND_CMD+=" --log-file=$LOG_FILE"
    $VALGRIND_CMD $EXECUTABLE_PATH $ADDITIONAL_ARGS
